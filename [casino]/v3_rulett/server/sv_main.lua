...
function giveChips(source, amount)
    ...
    if Player.Functions.AddItem("casinochips", amount, nil, nil, false, GetCurrentResourceName(), "", "", "")  then
        ...
        r_showNotification(source, "+ "..amount.." Chips")
    end
    -- YOUR CODE
end
...
AddEventHandler('casino:taskBetRulett', function(rulettIndex, betId, betAmount)
    ...
    if aktivRulettek[rulettIndex] ~= nil  then
        if aktivRulettek[rulettIndex].statusz  then
            return r_showNotification(src, _U('can_not_bet_because_started'))
        end

        local chipsAmount = getPlayerChips(src)
        if chipsAmount >= betAmount  then
            removeChips(src, betAmount)
            r_showNotification(src, _('placed_bet', betAmount))
            TriggerClientEvent('casino:nui:updateChips', src, getPlayerChips(src))
            ...
            r_showNotification(src, _U('not_enough_chips'))
        end
    else -- the table not existing on the serverside? error
        TriggerClientEvent('Notify', src, 'error', 'Hiba történt, nem létező rulettasztal szerveroldalon?')
    end
end)
