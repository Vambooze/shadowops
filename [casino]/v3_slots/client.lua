...
createSlots = function(index, data)
		...
		if state then
			...
			Citizen.CreateThread(function()
				while selectedSlot ~= nil do
					...
					helpText('~INPUT_CELLPHONE_CANCEL~ Stand up\n~INPUT_FRONTEND_RDOWN~ ~INPUT_JUMP~ Spin\n~INPUT_CONTEXT~ Change Camera\nBet: ~y~'..self.data.bet..'~w~ Chips')
				end
			end)
            Citizen.Wait(2000)
		else
	...
	self.spin = function(tickRate)
		...
		PlaySoundFromEntity(-1, "start_spin", self.tableObject, s, 1, 20)
		PlaySoundFromEntity(-1, "spinning", self.tableObject, s, 1, 20)
...
Citizen.CreateThread(function()
	while true do
		local letSleep = true
		--if not exports["v3_casino"]:isInsideCasino() then
			--Citizen.Wait(5000)
		--end
		if LocalPlayer.state.isLoggedIn and Framework ~= nil then
				...
				for k, v in pairs(Slots) do
					if DoesEntityExist(v.tableObject) then
						...
						if GetDistanceBetweenCoords(playerpos, Offset.x, Offset.y, Offset.z, true) <= 1.5 then
							...
							DrawText3Ds(closestChairData.position.x, closestChairData.position.y, closestChairData.position.z+1.2, '~g~E~w~ - Bet: '..v.data.bet)
							if IsControlJustPressed(0, 38) then
								Framework.Functions.TriggerCallback('casino:slots:isSeatUsed',function(used)
									if used then
									else
										ACTIVE_SLOT = v.index
						...
						for k,v in pairs(Config.Casino.SlotBets) do
							local playerPed = PlayerPedId()
							local objects = Framework.Functions.GetObjects()
							local closestDistance, closestObject = -1, -1
							local filter, coords = k, Config.Casino.Center
							if type(filter) == 'string' then
								if filter ~= '' then
