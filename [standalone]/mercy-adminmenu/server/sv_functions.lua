...
function IsPlayerBanned(source)
    local License = Mercy.Functions.GetIdentifier(source, 'license')
...
function GetPlayerFromIdentifier(Type, Identifier)
    ...
    for _, v in pairs(Mercy.Functions.GetPlayers()) do
...
function CreateLog(Source, Type, Log, Data)
    ...
    if Player ~= nil then
        local Steam = Mercy.Functions.GetIdentifier(src, "steam")
...
function AdminCheck(ServerId)
    ...
    if Player ~= nil then
        local Group = Mercy.Functions.GetPermission(ServerId)
