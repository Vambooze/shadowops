...
function ToggleMenu(IsUpdate)
    ...
    SendNUIMessage({
        ...
        Name = Mercy.Functions.GetPlayerData().name,
...
function DoPermsAction(Action, CommandId, Group)
    ...
    if Action == 'add' then
        if CommandId == 'all' then
            ...
            Mercy.Functions.Notify(Lang:t('info.perms_added_all'))
        end
    elseif Action == 'remove' then
        if CommandId == 'all' then -- Remove From Every Table
            if not Group then return Mercy.Functions.Notify(Lang:t('info.perms_group_exist')) end
            ...
            Mercy.Functions.Notify(Lang:t('info.perms_removed_all'))
        end
    end
    for p, q in pairs(Config.CommandList) do
        ...
        for j=1, #Command['Items'] do
            ...
            if not Item['UseKVPGroups'] then return Mercy.Functions.Notify(Lang:t('info.perms_disabled')) end
            if Groups ~= nil then
                if Action == 'add' then
                    if CommandId ~= 'all' and Groups[CommandId] ~= nil and Item['Id'] == CommandId then 
                        for k, v in pairs(Groups[CommandId]) do -- Check if Group already exists
                            if v == Group then 
                                return Mercy.Functions.Notify(Lang:t('info.perms_exist')) 
                            end
                        end
                        table.insert(Groups[CommandId], Group)
                        Mercy.Functions.Notify(Lang:t('info.perms_added'))
                    end
                elseif Action == 'remove' then
                    if CommandId ~= 'all' and Groups[CommandId] ~= nil and Item['Id'] == CommandId then
                        for k, v in pairs(Groups[CommandId]) do
                            if k == #Groups[CommandId]+1 and v ~= Group then return Mercy.Functions.Notify(Lang:t('info.perms_not_exist')) end
                            if v == Group then  
                                ...
                                Mercy.Functions.Notify(Lang:t('info.perms_removed'))
                            end
                        end
                    end
                elseif Action == 'list' then
                    if Groups[CommandId] ~= nil then 
                        if Item['Id'] == CommandId then
                            return Mercy.Functions.Notify(Lang:t('info.perms_list', { command = CommandId, groups = json.encode(Groups[CommandId]) }))
                        end
                    else
                        return Mercy.Functions.Notify(Lang:t('info.perms_command_exist'))
                    end
                end
            end
        end
...
function CreateLog(Type, Log, Data)
    ...
    Mercy.Functions.TriggerCallback('mc-admin/server/create-log', function(result)
    end, Type, Log, Data)
end
...
function GetServerConvar(Convar)
    local Prom = promise:new()
    Mercy.Functions.TriggerCallback('mc-adminmenu/server/get-convar', function(ReturnData)
        Prom:resolve(ReturnData)
    end, Convar)
    return Citizen.Await(Prom)
end
...
function GetPlayerReportFromName()
    local Player = Mercy.Functions.GetPlayerData()
    if Player ~= nil then
...
function GetPlayersInArea(Coords, Radius)
    local Prom = promise:new()
    Mercy.Functions.TriggerCallback('mc-admin/server/get-active-players-in-radius', function(Players)
...
function GetBans()
    local Prom = promise:new()
    Mercy.Functions.TriggerCallback("mc-admin/server/get-bans", function(Bans)
        Prom:resolve(Bans)
    end)
    return Citizen.Await(Prom)
end
...
function GetPlayers()
    local Prom = promise:new()
    Mercy.Functions.TriggerCallback("mc-admin/server/get-players", function(Players)
        Prom:resolve(Players)
    end)
    return Citizen.Await(Prom)
end
...
function GetLogs()
    local Prom = promise:new()
    Mercy.Functions.TriggerCallback("mc-admin/server/get-logs", function(Logs)
        Prom:resolve(Logs)
    end)
    return Citizen.Await(Prom)
end
...
function GetMoneyTypes()
    local Prom = promise:new()
    local MoneyTypes = {}
    local Types = Mercy.Config.Money.MoneyTypes
    if Types ~= nil then
...
function GetJobs()
    ...
    if Jobs ~= nil then
        ...
        DebugPrint('jobs', 'Could not access QBShared.Jobs, please check if you have the latest version of QBCore or the right name for the jobs shared.')
    end
    return Citizen.Await(Prom)
end
...
function GetAddonVehicles()
    ...
    if Vehicles ~= nil then
        for k, v in pairs(Vehicles) do
            ...
            local VehBrand = v['brand'] ~= nil and v['brand'] or 'Unknown Brand'
            local VehModel = v['model'] ~= nil and v['model'] or 'Unknown Model'
        ...
        DebugPrint('vehicles', 'Could not access QBShared.Vehicles, please check if you have the latest version of QBCore or the right name for the vehicles shared.')
    end
    return Citizen.Await(Prom)
end
