...
function ToggleSpectate(targetPed, targetPlayerId)
    ...
    if SpectateEnabled then
        ...
        clearGamerTagInfo()
        DoScreenFadeIn(500)
        Mercy.Functions.Notify(Lang:t('spectate.toggled', {toggled_spectate = Lang:t('commands.stopped'), player = GetPlayerName(StoredTargetPlayerId)..' ('..GetPlayerServerId(StoredTargetPlayerId)..')'}))
        StoredTargetPed = nil
    else
        ...
        DoScreenFadeIn(500)
        Mercy.Functions.Notify(Lang:t('spectate.toggled', {toggled_spectate = Lang:t('commands.started'), player = GetPlayerName(StoredTargetPlayerId)..' ('..GetPlayerServerId(StoredTargetPlayerId)..')'}))
...
function cleanupFailedResolve()
    ...
    Mercy.Functions.Notify(Lang:t('spectate.stopped_not_found'), 'error')
end
...
RegisterNetEvent('Mercy/client/specPlayer', function(targetServerId, coords)
    ...
    if targetPlayerId == PlayerId() then
        return Mercy.Functions.Notify(Lang:t('spectate.self'), 'error')
    end
    DoScreenFadeOut(500)
    while not IsScreenFadedOut() do Wait(0) end
