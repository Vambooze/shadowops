...
QBCore.Functions.CreateCallback('kevin-deliveries:hasPackage', function(source, cb)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
	local hasPackage = Player.Functions.GetItemByName(Config.PackageName)
    if hasPackage then
        cb(true)
    else
        cb(false)
...
RegisterServerEvent('kevin-deliveries:RemovePackage', function()
	local src = source
	local Player = QBCore.Functions.GetPlayer(src)
	Player.Functions.RemoveItem(Config.PackageName, 1, false)
	TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items[Config.PackageName], "remove")
	Wait(2500)
end)
