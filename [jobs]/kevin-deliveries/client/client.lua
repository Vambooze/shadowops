            ...
            options = {
                {
                    ...
                    action = function()
                        TriggerEvent('kevin-deliveries:start')
                    end,
                },
                {
                    icon = 'fas fa-circle',
                    ...
                    action = function()
                        TriggerEvent('kevin-deliveries:finish')
                    end,
                },
                {
                    type = 'client',
...
function StartRun()
    ...
    if not IsAnyVehicleNearPoint(vehspawn.x, vehspawn.y, vehspawn.z, 3.0) then
        ...
        elseif Config.FuelScript == 'lj-fuel' then
            exports['lj-fuel']:SetFuel(JobVeh, 100)
        end
        SetTrunkItemInfo()
...
RegisterNetEvent('kevin-deliveries:Deliver', function ()
    if not PackageDelivered then
        ...
        if deliveredboxes == givenboxes then
            ...
            elseif Config.Phone == 'gks' then
                TriggerEvent('gksphone:notifi', {title = 'CURRENT TASK', message = 'Return to the docks and return the truck', img= '/html/static/img/icons/messages.png'})
            end
            RemoveBlip(DestBlip)
            Return()
        end
