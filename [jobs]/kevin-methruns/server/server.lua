		...
		for k, v in pairs(GetStartItems()) do
			if v.name == Config.StartItem then
				if v.amount == Config.RequiredAmount then
					TriggerClientEvent('kevin-methruns:client:verifypackages', src)
...
RegisterNetEvent('kevin-methruns:server:giveproducts', function ()
	...
	if Player.PlayerData.metadata['methruns'] <= Config.LevelRep then
		...
		TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items[Config.LowRepItem], "add")
	elseif Player.PlayerData.metadata['methruns'] >= Config.LevelRep then
        Player.Functions.AddItem(Config.HighRepItem, 1, false)
		TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items[Config.HighRepItem], "add")
	end
end)

RegisterNetEvent('kevin-methruns:coptracker', function (VehPos)
	TriggerClientEvent('kevin-methruns:coptracker', -1, VehPos)
end)
...
QBCore.Functions.CreateCallback('kevin-methruns:server:hasItems', function(source, cb)
	...
	if Player.PlayerData.metadata['methruns'] <= Config.LevelRep then
		...
		if hasItems then
			...
			TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items[Config.LowRepItem], "remove")
			cb(true)
		else
			cb(false)
		end
	elseif Player.PlayerData.metadata['methruns'] >= Config.LevelRep then
		...
		if hasItems then
			...
			TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items[Config.HighRepItem], "remove")
			cb(true)
		else
			cb(false)
		end
...
RegisterNetEvent('kevin-methruns:server:Rewards', function()
	...
	TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items['methcase'], "remove")
...
RegisterNetEvent('kevin-methruns:server:GetRep', function ()
	...
	TriggerClientEvent('QBCore:Notify', src, 'Current Reputation: '..rep, 'primary', 5000)
end)
...
RegisterNetEvent('kevin-methruns:server:getCase', function ()
	...
	TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items['methcase'], "add")
end)
