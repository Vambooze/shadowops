...
RegisterNetEvent('police:client:VehicleSubMenu', function(data)
    ...
    local SubMenu = {}
    if data.out then
        SubMenu[#SubMenu+1] = {opthead = Lang:t('menu.returncar', {vehicle = vehLabel}), optdesc = "", opticon = 'car', optparams = {event = "police:client:ReturnVehicle", args = {car = data.car}}}
        table.remove(PDCar, data.tableid)
    else
        SubMenu[#SubMenu+1] = {opthead = Lang:t('menu.takeout', {vehicle = vehLabel}), optdesc = Lang:t('menu.takeoutfor', {vehicle = data.price}), opticon = 'car', optparams = {event = "police:client:TakeOutVehicle", args = {vehicle = data.vehicle, currentSelection = data.currentSelection, livery = data.livery, price = data.price}}}
    end
    local newHeader = {
        disabled = true,
        header = Lang:t('menu.vehmenu', {vehicle = vehLabel}),
        headerid = 'police_vehicle_submenu', -- unique
        desc = Lang:t('menu.vehmenudesc'),
        icon = "retweet"
    }
...
RegisterNetEvent('qb-police:client:quickGiveItems', function()
    ...
    for _, v in pairs(Config.Items.items) do
        for i=1, #v.authorizedJobGrades do
            if v.authorizedJobGrades[i] == PlayerJob.grade.level and v.quick.equip then
                TriggerServerEvent('police:server:AddRemove', v.name, v.quick.amount, "add", source, SecKey)
            end
        end
    end
end)
...
RegisterNetEvent('police:client:ChangeLivery', function(data)
    ...
    if oldlivery then
		...
		if GetVehicleLivery(vehicle) == tonumber(data.id) then
            QBCore.Functions.Notify(Lang:t('error.alreadyinstall', {value = data.id}), "error")
			return
		end
	else
		if modName == "NULL" then modName = "Stock" end
		if GetVehicleMod(vehicle, 48) == tonumber(data.id) then
            QBCore.Functions.Notify(Lang:t('error.alreadyinstall', {value = modName}), "error")
			return
		end
	end
	if oldlivery then
...
RegisterNetEvent('police:client:ChangeExtra', function(data)
    ...
    if IsVehicleExtraTurnedOn(veh, data.id) then
        ...
        QBCore.Functions.Notify(Lang:t('error.extradeactive', {value = data.id}), 'error', 2500)
    else
        SetVehicleExtra(veh, data.id, 0)
        QBCore.Functions.Notify(Lang:t('success.extraactive', {value = data.id}), 'success', 2500)
    end
    local data = {}
    data.vehicle = veh
...
RegisterCommand('liverymenu', function()
    if not PlayerJob.type == "leo" then QBCore.Functions.Notify(Lang:t('error.cantusemenu'), "error") return end
    local vehicle = nil
	if IsPedInAnyVehicle(PlayerPedId(), false) then	
