...
const BuyVehicle: React.FC = () => {
  ...
  const calculateTotalPrice = useCallback((): number => {
    const vehiclePrice = selectedCar.price;
    let platePrice = 0;
    ...
    let blackMoneyPrice = 1;
    if (paymentMethod == "black_money") {
      if (OpenedGallery?.buyWithBlackMoney.active) {
        blackMoneyPrice = OpenedGallery.buyWithBlackMoney.multiplier ?? 1;
      }
    }
    return (vehiclePrice + platePrice) * blackMoneyPrice;
